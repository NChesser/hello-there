...
// Icons
import { Edit2, Trash2, Save, X, Star, Plus, Send, MapPin, Heart, Tag, Cake, MessageCircle, Sprout, Calendar, Sparkles } from 'lucide-react';
...
const PersonDetailScreen = ({ personId, onBack }: PersonDetailScreenProps) => {
    ...
    const [detailTab, setDetailTab] = useState < 'about' | 'moments' | 'growth' > ('about');

    if (!person) {
        return (
            <ScreenContainer>
                <div className="text-center py-12">
                    <p className="text-amber-700 dark:text-gray-400">Person not found.</p>
                    <button
                        onClick={onBack}
            ...
            peopleMet: people.map(p =>
                p.id === personId
                    ? {
                        ...
                        somethingInteresting: editInteresting.trim() || undefined,
                        whereMet: editWhereMet.trim() || undefined,
                        thingsTheyLike: editThingsTheyLike.length > 0 ? editThingsTheyLike : undefined,
    ...
    const addInteraction = () => {
        ...
        setUserProgress({
            peopleMet: people.map(p =>
                p.id === personId
                    ? { ...p, interactions: [interaction, ...(p.interactions || [])] }
                    : p
            ),
        });
    ...
    const addNote = () => {
        ...
        setUserProgress({
            peopleMet: people.map(p =>
                p.id === personId
                    ? { ...p, personalNotes: [note, ...(p.personalNotes || [])] }
                    : p
            ),
        });
        setNewNoteText('');
    ...
    const removeNote = (noteId: string) => {
        setUserProgress({
            peopleMet: people.map(p =>
                p.id === personId
                    ? { ...p, personalNotes: (p.personalNotes || []).filter(n => n.id !== noteId) }
                    : p
            ),
        });
    };
    ...
    const removePerson = async () => {
        ...
        if (confirmed) {
            setUserProgress({ peopleMet: people.filter(p => p.id !== personId) });
            onBack();
        }
    };
    ...
    return (
        <ScreenContainer>
            ...
            {/* Person header */}
            <div className="border-2 rounded-2xl shadow-sm overflow-hidden bg-gradient-to-br from-white to-amber-50 border-amber-200 dark:from-gray-800 dark:to-gray-750 dark:border-gray-700">
                ...
                <div className="flex items-start gap-4 p-5 pb-3">
                    ...
                    <div className="flex-1 min-w-0">
                        {isEditing ? (
                            ...
                            <h2 className="text-xl font-bold text-amber-900 dark:text-amber-200 truncate">{person.name}</h2>
                        )}
                        {person.tags && person.tags.length > 0 && (
                            <div className="flex gap-1 mt-1 flex-wrap">
                                {person.tags.map(tag => (
                                    <span key={tag} className="text-[10px] px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-gray-600 dark:text-amber-300 font-medium">
                                        {tag}
                                    </span>
                                ))}
                            </div>
                        ...
                        <div className="flex items-center gap-3 flex-wrap mt-2">
                            ...
                            {person.birthday && (
                                <span className="inline-flex items-center gap-1 text-xs text-amber-500 dark:text-gray-400">
                                    <Cake size={11} /> {new Date(person.birthday + 'T00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                                </span>
                            )}
                            {person.whereMet && (
                                <span className="inline-flex items-center gap-1 text-xs text-amber-500 dark:text-gray-400">
                ...
                {!isEditing && person.somethingInteresting && (
                    <div className="mx-5 mb-4 rounded-lg px-4 py-3 border-l-4 border-blue-300 bg-blue-50/70 dark:bg-blue-900/15 dark:border-blue-600">
                        <p className="text-xs font-medium mb-0.5 text-blue-600 dark:text-blue-400"><Sparkles size={11} className="inline mr-0.5" /> Something Interesting</p>
                        <p className="text-sm italic text-blue-800 dark:text-blue-300">"{person.somethingInteresting}"</p>
                    </div>
                )}
                ...
                {isEditing && (
                    <div className="space-y-3 px-5 pb-5 pt-3 border-t border-amber-200 dark:border-gray-600">
                        ...
                        <div>
                            <label className="block text-[11px] font-medium mb-1 text-amber-700 dark:text-amber-300">Something Interesting</label>
                            <textarea
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                <label className="block text-[11px] font-medium mb-1 text-amber-700 dark:text-amber-300">
                                    <MapPin size={11} className="inline mr-0.5" /> Where Met
                                </label>
                                <input
                                    type="text"
                                    value={editWhereMet}
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {THINGS_THEY_LIKE_OPTIONS.map(like => (
                                    <button
                                        key={like}
                                        type="button"
                                        onClick={() => setEditThingsTheyLike(prev =>
                                            prev.includes(like) ? prev.filter(l => l !== like) : [...prev, like]
                                        )}
                                        className={`text-[11px] px-2.5 py-1 rounded-full font-medium transition-all active:scale-95 ${editThingsTheyLike.includes(like)
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {RELATIONSHIP_TAGS.map(tag => (
                                    <button
                                        ...
                                        className={`text-[11px] px-2.5 py-1 rounded-full font-medium transition-all active:scale-95 ${editTags.includes(tag)
            ...
            <TabBar
                tabs={[
                    { id: 'about' as const, label: 'About', icon: Heart },
                    {
                        ...
                        label: 'Moments',
                        icon: MessageCircle,
                        badge: (person.interactions?.length ?? 0) > 0 ? (
                    ...
                    },
                    {
                        ...
                        label: 'Growth',
                        icon: Sprout,
                        badge: (person.personalNotes?.length ?? 0) > 0 ? (
                    ...
                    <div className="space-y-3">
                        {person.whereMet && (
                            <div className="rounded-lg p-4 border bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800">
                                ...
                                <p className="text-sm text-green-900 dark:text-green-300">{person.whereMet}</p>
                            </div>
                        )}

                        {person.thingsTheyLike && person.thingsTheyLike.length > 0 && (
                            <div className="rounded-lg p-4 border bg-purple-50 border-purple-200 dark:bg-purple-900/20 dark:border-purple-800">
                                ...
                                <div className="flex flex-wrap gap-1.5 mt-1">
                                    {person.thingsTheyLike.map(like => (
                                        <span key={like} className="text-xs px-2.5 py-1 rounded-full bg-purple-100 text-purple-700 dark:bg-purple-800/40 dark:text-purple-300 font-medium">
                                            {like}
                                        </span>
                                    ))}
                                </div>
                        ...
                        {person.birthday && (
                            <div className="rounded-lg p-4 border bg-pink-50 border-pink-200 dark:bg-pink-900/20 dark:border-pink-800">
                                ...
                                <p className="text-sm text-pink-900 dark:text-pink-300">
                                    {new Date(person.birthday + 'T00:00').toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}
                                </p>
                            </div>
                        )}

                        {!person.whereMet && !person.birthday && (!person.thingsTheyLike || person.thingsTheyLike.length === 0) && (
                            <div className="rounded-lg p-4 border border-dashed border-amber-200 dark:border-gray-600">
                                <p className="text-sm text-amber-500 dark:text-gray-500 text-center">No details yet — tap Edit to add some</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="border-2 rounded-2xl p-5 shadow-sm mt-3 bg-white border-amber-200 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    {isAddingInteraction && (
                        <div className="mb-4 flex gap-2">
                            ...
                            <button
                                onClick={() => { setIsAddingInteraction(false); setNewInteractionText(''); }}
                    ...
                    {(!person.interactions || person.interactions.length === 0) ? (
                        <div className="rounded-lg p-6 text-center border border-dashed border-amber-200 dark:border-gray-600">
                            <p className="text-sm text-amber-500 dark:text-gray-500">No interactions logged yet</p>
                            <p className="text-xs mt-1 text-amber-400 dark:text-gray-600">Tap “Add” to record your first one!</p>
                        </div>
                    ) : (
                        <div className="relative">
                            ...
                            <div className="space-y-3">
                                {person.interactions.map((interaction) => (
                                    <div key={interaction.id} className="flex gap-3 group relative">
                                        {/* Dot */}
                                        ...
                                        <div className="flex-1 rounded-lg p-3 border transition-colors bg-amber-50/50 border-amber-100 dark:bg-gray-700/30 dark:border-gray-600">
                                            <p className="text-sm text-amber-900 dark:text-gray-200">{interaction.text}</p>
                                            <p className="text-xs mt-1 text-amber-400 dark:text-gray-500">{interaction.date}</p>
                                        </div>
            ...
            {detailTab === 'growth' && (
                <div className="border-2 rounded-2xl p-5 shadow-sm mt-3 bg-white border-amber-200 dark:bg-gray-800 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-base font-semibold text-amber-900 dark:text-amber-200 flex items-center gap-2">
                            <Sprout size={16} className="text-green-600 dark:text-green-400" />
                            Growth
                    ...
                    {/* Notes list */}
                    {(!person.personalNotes || person.personalNotes.length === 0) ? (
                        <div className="rounded-lg p-6 text-center border border-dashed border-amber-200 dark:border-gray-600">
                            <p className="text-sm text-amber-500 dark:text-gray-500">No notes yet</p>
                            <p className="text-xs mt-1 text-amber-400 dark:text-gray-600">Tap "Add" to jot down a thought or reflection</p>
                        </div>
                    ) : (
                            ...
                            {person.personalNotes.map((note) => (
                                <div key={note.id} className="flex gap-2 group">
                                    <div className="flex-1 rounded-lg p-3 border transition-colors bg-green-50/50 border-green-100 dark:bg-gray-700/30 dark:border-gray-600">
                                        <p className="text-sm text-amber-900 dark:text-gray-200">{note.text}</p>
                                    </div>
                                    <button
